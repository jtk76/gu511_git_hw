...
def cross_validate_scores(pipelines, X, y, cv=None)
    ...
    with warnings.catch_warnings()
        ...
        LOGGER.debug('cross-validation log loss resultsn{}'.format(agg_results))
...
def load_adult_income_data(test_size=0.2, random_state=1337)
    ...
    repository](httparchive.ics.uci.edumlindex.php).
    we could use the requests library to download and parse the column names
    (available [here](httparchive.ics.uci.edumlmachine-learning-databasesadultadult.names)),
    ...
    df = pd.read_csv(
        'httparchive.ics.uci.edumlmachine-learning-databasesadultadult.data',
        names=columns,
        delimiter=', ',
        index_col=False,
        engine='python'
    ...
    # replace our string-valued target column with numerical 01 values
